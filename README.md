...
The reviewed maintained surface now contains 148 exact numeric enums and two integer-valued type unions in `BabylonjsBindings.Enums`, 69 exact string enums and resolved string-literal unions in `BabylonjsBindings.StringEnums`, 24 complete dependency-closed object aliases in `BabylonjsBindings.ObjectTypes`, eleven primitive/array/callback/generic aliases in `BabylonjsBindings.TypeAliases`, 221 dependency-closed interfaces plus exact readonly projections for eleven `DeepImmutable<T>` structural views and 27 `Partial<T>` views in `BabylonjsBindings.SimpleInterfaces`, 126 dependency-closed runtime classes in `BabylonjsBindings.SimpleClasses`, 799 dependency-closed runtime functions in `BabylonjsBindings.SimpleFunctions`, and 444 dependency-closed runtime variables/constants in `BabylonjsBindings.SimpleVariables` (1,844 of 4,323 locked exports). Mutually recursive interface/class dependencies are compiled through the generated `SimpleTypes.fs` build projection while their reviewed source fragments remain independently auditable. Their proposals and per-export promotion manifests are regenerated from the declaration lock and checked for drift. `npm run test:candidate` independently Fable-compiles the full review candidate, resolves emitted imports, executes Node and Chromium smokes, dynamically verifies every maintained class, function, and variable export, packs a local-only candidate package, and consumes that package from a clean temporary project. These proofs authorize incremental review; they do not make blocked or lossy exports complete.
