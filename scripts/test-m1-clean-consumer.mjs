...
await run("dotnet", ["build", "src/BabylonjsBindings/BabylonjsBindings.fsproj", "--no-restore", "--configuration", "Release", "-p:PackageVersion=0.1.1"]);
await run("dotnet", ["pack", "src/BabylonjsBindings/BabylonjsBindings.fsproj", "--no-restore", "--no-build", "--configuration", "Release", "--output", packageDirectory, "-p:PackageVersion=0.1.1"]);
const packagePath = resolve(packageDirectory, "BabylonjsBindings.0.1.1.nupkg");
...
try {
  ...
  await writeFile(resolve(consumer, "Consumer.fsproj"), `<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><OutputType>Exe</OutputType><TargetFramework>netstandard2.1</TargetFramework></PropertyGroup><ItemGroup><Compile Include="Program.fs"/><PackageReference Include="BabylonjsBindings" Version="0.1.1"/></ItemGroup></Project>\n`);
...
`);
  ...
  console.log(JSON.stringify({ package: packagePath, packageVersion: "0.1.1", npm: { core: "9.19.0", loaders: "9.19.0" }, fable: "5.13.0", result: "pass" }, null, 2));
} finally {
