...
// green. `BabylonjsBindings` is rewritten to the product's name on instantiation, as in
// `lifecycle-evidence.mjs`, so these paths name the delivered projects rather than the template's.
for (const file of ["package-lock.json", "declaration-lock.json", "coverage-and-drift.json", "scaffold-provenance.json", "bindings-evidence.yml", "src/BabylonjsBindings/packages.lock.json", "tests/BabylonjsBindings.CompileTests/packages.lock.json"]) if (!existsSync(file)) throw new Error(`missing required evidence artifact: ${file}`);
