...
try {
  const url = `http://127.0.0.1:${server.address().port}/runtime/browser/m2-xr.html`;
  ...
  if (!browser.stdout.includes("<output>M2 deterministic WebXR smoke passed</output>")) throw new Error(`Chromium XR stub smoke failed\n${browser.stdout}\n${browser.stderr}`);
  console.log(`M2 deterministic WebXR smoke passed (${version}; no physical device)`);
