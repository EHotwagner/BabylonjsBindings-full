...
As of 2026-08-31T07:29:58Z, the maintained binding covers 4,237 of 4,323 locked exports (98.0%): 85 exports are blocked, one loader-registration side effect is intentionally runtime-only, and none are classified as lossy. The remaining work is concentrated in eleven related type-system families. This document records the exact obstacles, the preferred Fable projections, the order in which they should be implemented, and the evidence required before an export can move from `blocked` to `typed`.
...
| [ ] M3 — higher-order runtime APIs | Remaining miscellaneous functions (5) and material mixins (3) | 8 | 4,287 | Arity policy recorded; emitted decorator/mixin runtime tests; coverage 35 blocked.<br>Progress: implementation PR #9 squash-merged as `4c1b5b73e02b4e2a137e2a3c2f9ecb0842278629`; critique `reviews/roadmap/roadmap-missing-exports-m3-higher-order-runtime-apis.json` (1 repair round, pass); feedback `feedback/2026-08-31-BabylonjsBindings-full-m3-higher-order-runtime-apis.md` and audit; durable SDD verdict `readiness/005-m3-higher-order-runtime-apis/ship-verdict.json`, host command `npm run check:m3-sdd-readiness`; arity policy 0–10 with broader fixed-callable context 17; exact coverage 4,287 typed / 35 blocked / 1 runtime-only / 0 lossy; 0.1.4 release in progress. |
...
Until then, the current 98.0% package is highly usable but the full-coverage release gate should remain red.
